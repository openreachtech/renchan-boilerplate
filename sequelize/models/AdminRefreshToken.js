 ...
 *
 * The long-lived half of the management UI's credential pair. It reaches the browser only as an
 * `HttpOnly` cookie — a different cookie, on a different path, from the one the customer UI holds
 ...
 * exists at all.
 */
export default class AdminRefreshToken extends RenchanModel {
  /** @override */
   ...
   * to the browser, and only the digest belongs in this table. The session clerk names the
   * principal `userId` for both audiences; here it is the id of the admin the session belongs to,
   * stored in `AdminId`.
   *
   * @param {{
   *   userId: number
   ...
   *   expiredAt?: Date
   * }} params - Parameters.
   * @returns {AdminRefreshToken} - Built entity, not saved yet.
   */
   ...
   * {@link AdminRefreshToken.buildWithGeneratedAttributes}. The shared `SessionClerk` holds this
   ...
   *
   * @returns {number} - The user id (here, the admin id).
   */
  extractUserId () {
    return /** @type {number} */ (
      this.get('AdminId')
...
/**
 * @typedef {AdminRefreshTokenEntity & {
 *   Admin: import('./Admin').AdminEntity
 * }} AdminRefreshTokenAssociatedEntity
 */
